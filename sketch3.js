...
treasuresprite=createSprite(canW/2+200,500)
...
roof=createSprite(1300,canH/2+5,4*canW,10)
  roof.visible=true;
...
if(touches.length>0||keyDown("space")){
 ...
 if(ninja.y>700){
  ...
  if(ninja.isTouching(roof)&&ninja.y>=340) {
      var token=1;
 ...
 else if(villan2group[x].isTouching(ninja)&&token===1&&ninja.isTouching(roof))
    {
    gameState="gameover"
...
for(x=0;x<coingroup.length;x++){
 ...
 if(distance>=1000){
   fill("black")
   textSize(30)
   text("LEVEL2 COMPLETED",ninja.x,canH/2-200)
   text("UNLOCK TREASURE",ninja.x,canH/2-130)
 // icon2.visible=true
ninja.velocityX=0;
ninja.velocityY=0;
//console.log(icon2.x)
...
ninja.velocityY=ninja.velocityY+1 
console.log(ninja.x)
if(ninja.x>mainvillsprite.x+200){
  bgsound.stop();
cheersound.play();
...
villan.velocityX=-(8+3*distance/100)
villan.scale=1.3
villan.debug=false
